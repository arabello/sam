...
class AIRelationJSONView(callback: String => Unit) extends AIRelationScreenView{
	...
	override def receiveUpdate(viewModel: AIRelationViewModel): Unit = {
		...
		val datasets =
			(for(point <- viewModel.points) yield Dataset(point.label, Seq(Point(point.x, point.y, pointRadius)), RGBAColor.randomWithAlpha(0.2f).toString)).toSeq :+
		...
		val chart = Chart(viewModel.title, viewModel.xAxis, viewModel.yAxis, datasets)
